...
## Advertising State
a device in this state sends out advertising packets at a fixed interval `Advertising Interval`. They do so in one of the three `Primary Advertising Channels`:
...
Where `S_t` is the ratio between successful packets over all packets (successful + failed) within a timeframe `t`.
...
[IPv6 header is compressed by using information present in t802.15.4 MAC header](https://datatracker.ietf.org/doc/html/rfc4944#section-10).
...
This makes it impossible for a downward traversal, from root to leaves. `DAO` messages are sent by leaf nodes when they first join, informing parents about reachability (they call it prefix reachability and I don't know if there's a significant difference). This process terminates once the root is aware of the hierarchy.